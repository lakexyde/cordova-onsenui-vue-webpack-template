module.exports = function (ctx) {
	...
	const sys = {
		...
		checkNodeModules() {
			...
			console.log('Checking if node modules are installed...')
			...
			if (!fs.existsSync(nodeModulesPath) || !fs.existsSync(path.resolve(nodeModulesPath, 'cheerio/'))) {
				...
				exec('npm i', {cwd: pRoot, maxBuffer: 1024 * 1024 * 5}, (error) => {
					if (error) {
						console.error(`Error occured during npm install: ${error}`);
						defer.reject(new Error(`Error occured during npm install: ${error}`))
					}
